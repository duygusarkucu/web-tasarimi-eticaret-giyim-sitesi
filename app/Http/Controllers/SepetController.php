...
class SepetController extends Controller
{
    ...
    public function store1($id,$user, Request $request){

        $sepet_is = Sepet::where('product_id',$id)->where('user_id',$user)->get();
        if (count($sepet_is)>0) {
            $sepet = $sepet_is[0];
            $sepet->piece = $sepet->piece + $request->piece ;
            $sepet->color = $request->color  ;
            $sepet->beden = $request->beden  ;
            $sepet->save();
        } else {
            $sepet = new Sepet();
            $sepet->user_id = $user;
            $sepet->product_id = $id;
            $sepet->piece =  $request->piece ?  $request->piece : 1;
            $sepet->color = $request->color  ;
            $sepet->beden = $request->beden  ;
            $sepet->save();
        }
